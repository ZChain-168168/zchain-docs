...
# ZRC20 Token Transfer

So far, we've set up bridge to exchange assets/data between Zchains PoS and Zchains chain. This section will guide you to set up an ERC20 bridge and send tokens between different blockchains.
...
### Step 2: Transfer ZRC20 Token

We will send ZRC20 Tokens from the Zchains PoS chain to the Zchains Edge chain.
...
To transfer tokens to Zchains chains, you will call `deposit`.
...
```bash
INFO[11-19|08:15:58] Handling fungible deposit event          chain=Zchains dest=100 nonce=1
...
Once the execution transaction has been successful, you will get tokens in the Zchains chain.
